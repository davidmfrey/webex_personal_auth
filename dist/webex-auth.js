...
class TokenManager {
    ...
    async saveToken(token) {
        ...
        const lines = existingContent.split('\n').filter(line => !line.startsWith('WEBEX_ACCESS_TOKEN=') &&
    ...
    async validatePersonalToken(token) {
        try {
            console.log('   📡 Making API call to https://webexapis.com/v1/people/me');
            const response = await axios_1.default.get('https://webexapis.com/v1/people/me', {
                headers: {
    ...
    displayPersonalTokenInfo(token) {
    ...
    isValidTokenFormat(text) {
        ...
        if (!webexTokenPattern.test(cleaned))
            return false;
...
//# sourceMappingURL=webex-auth.js.map